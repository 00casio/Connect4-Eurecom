...
if [ $? -ne 0 ]; then
  echo "Could not try to install libcairomm, and libgirepository"
  echo "Look for them and try to install them"
else
  echo "We need your password to install prerequisite"
  echo "sudo apt install libcairomm-1.0-dev libgirepository1.0-dev"
  sudo apt install libcairomm-1.0-dev libgirepository1.0-dev
fi
